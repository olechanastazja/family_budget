...
class TestBudget:
    ...
    def test_read_permissions(
        self, api_client, user2, budget_user1_only, budget_user1_shared_with_user2
    ):
    ...
    def test_write_permissions(
        ...
        api_client,
        request,
        budget_user1_shared_with_user2,
        user_fixture,
        status_code,
    ):
    ...
    def test_budget_recalculate(
        ...
        item_response = client.post(
            self.BUDGET_ITEM_ENDPOINT, data=budget_item_data, format="json"
        )
        ...
        if item_type == BudgetItem.EXPENSE:
            assert (
                float(end_budget_total)
                == initial_budget_total - budget_item_data["amount"]
            )
