...
class TestBudgetItem:
    ...
    def test_write_permissions(
            self, api_client, request, budget_expense, user_fixture,
            status_code
    ):
    ...
    def test_filter_by_budget_and_category(self, api_client, user, budget_expense, budget_user1_only, categories):
        url = f'{self.ENDPOINT}?budget={budget_user1_only.id}&category={categories[0].id}'
        client = api_client()
        client.force_authenticate(user=user)
        response = client.get(url)
        assert response.status_code == status.HTTP_200_OK
